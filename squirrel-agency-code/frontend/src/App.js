...
const serviceDetails = {
  "website-development": {
    ...
    points: [
      ["STRATEGY & IA", "Sitemap, messaging hierarchy and page jobs defined before a single layout is drawn."],
      ["DESIGN SYSTEM", "Typography, colour, components and motion that scale across every page."],
      ["BUILD & PERFORMANCE", "Clean front-end engineering, responsive layouts, SEO basics and conversion-minded CTAs."],
      ["LAUNCH SUPPORT", "QA, analytics setup, handover docs and a clear path for future updates."],
    ],
...
function Image({ src, alt, className = "", eager = false }) {
  return (
    <img
      ...
      onError={(e) => { e.currentTarget.src = images.workspace; }}
    />
  );
}
function Reveal({ children, className = "", delay = 0, ...props }) {
  return (
    <motion.div
      ...
      transition={{ duration: 0.55, delay: Math.min(delay, 0.12), ease }}
      {...props}
    >
      {children}
    </motion.div>
...
function SectionHead({ title, text }) {
  return (
    <div className="section-heading">
      ...
      {text && <Reveal delay={0.08}><p>{text}</p></Reveal>}
    </div>
  );
}
function Logo() { return <img className="logo-mark" src={LOGO} alt="Squirrel Agency — creative minds smarter results" width="160" height="60" decoding="async" />; }
...
function Footer() { return <footer className="footer"><div className="footer-top"><Logo /><div className="footer-claim" data-testid="footer-claim">Technology.<br />Creativity.<br /><em>Performance.</em></div></div><div className="footer-grid"><div><span className="footer-label">Navigate</span>{[["Home", "/"], ["Services", "/services"], ["About", "/about"], ["Work", "/work"], ["Insights", "/insights"], ["Case Studies", "/case-studies"], ["Contact", "/contact"]].map(([x, p]) => <Link key={p} to={p} data-testid={`footer-${x.toLowerCase().replaceAll(" ", "-")}`}>{x}</Link>)}</div><div><span className="footer-label">Disciplines</span>{services.slice(0, 6).map((s) => <Link key={s[3]} to={`/services/${s[3]}`} data-testid={`footer-service-${s[3]}`}>{s[1]}</Link>)}</div><div><span className="footer-label">Start a conversation</span><a href="tel:+919277477048" data-testid="footer-phone">+91 92774 77048</a><a href="mailto:info.thesquirrelagency@gmail.com" data-testid="footer-email">info.thesquirrelagency@gmail.com</a><a href="https://instagram.com/thesquirrel_agency" target="_blank" rel="noreferrer" data-testid="footer-instagram">@thesquirrel_agency</a><span className="footer-address" data-testid="footer-location">Based in Kanpur · Serving clients worldwide</span></div></div><div className="footer-bottom"><span data-testid="footer-copyright">© 2026 The Squirrel Agency. All rights reserved.</span><div><Link to="/privacy" data-testid="footer-privacy">Privacy Policy</Link><Link to="/terms" data-testid="footer-terms">Terms</Link></div></div></footer>; }
function Hero() {
...
function NewsletterBox() {
  ...
  return <section className="light newsletter" data-testid="newsletter"><Reveal><h2>New notes,<br /><em>first in your inbox.</em></h2><p>One short email when a new insight goes live. No noise, unsubscribe anytime.</p><form onSubmit={subscribe} className="newsletter-form" data-testid="newsletter-form"><input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@company.com" aria-label="Email address" data-testid="newsletter-email" /><button className="button button-accent" type="submit" disabled={state === "saving"} data-testid="newsletter-submit">{state === "saving" ? "Subscribing..." : "Subscribe ↗"}</button></form>{state === "done" && <p className="form-success" data-testid="newsletter-success">You're on the list. See you in the inbox.</p>}{state === "error" && <p className="form-error" data-testid="newsletter-error">Could not subscribe right now — please try again.</p>}</Reveal></section>;}
