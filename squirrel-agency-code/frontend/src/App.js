...
function Marketplace() { return <section className="section light marketplace"><Reveal><span className="eyebrow">07 / MARKETPLACE GROWTH</span><h2>Built to sell<br /><em>everywhere.</em></h2><p>From title optimization to image direction, we make the product easier to find, trust and choose.</p></Reveal><Reveal className="market-grid" delay={0.1}>{[["AMAZON", "Title optimization · A+ content", img("mkt-amazon")], ["FLIPKART", "Category mapping · Listing health", img("mkt-flipkart")], ["MYNTRA", "Fashion attributes · Studio imagery", img("mkt-myntra")]].map(([x, y, src]) => <div className="market-card" key={x} data-testid={`marketplace-${x.toLowerCase()}`}><Image src={src} alt={`${x} marketplace product visual`} /><div><strong>{x}</strong><span>{y}</span></div></div>)}</Reveal></section>; }

function Process() { return <section className="section light process-section"><SectionHead eyebrow="08 / THE METHOD" title={<>How we<br /><em>work.</em></>} text="Good work is not a reveal. It is a rhythm — clear questions, brave decisions, measured iteration." /><div className="timeline">{["Discover", "Strategize", "Design", "Develop", "Launch", "Optimize", "Scale"].map((x, i) => <Reveal className="timeline-step" key={x} delay={i * 0.05} data-testid={`process-step-${x.toLowerCase()}`}><span>0{i + 1}</span><strong>{x}</strong><p>{["Find the real problem underneath the brief.", "Make the point of view impossible to miss.", "Shape the system, story and experience.", "Build it cleanly, quickly and to last.", "Put the work in front of the right people.", "Read the signal, refine the response.", "Turn momentum into a durable advantage."][i]}</p></Reveal>)}</div></section>; }

function Testimonials() { return <section className="section testimonials-section"><SectionHead eyebrow="09 / KIND WORDS" title={<>Trusted in<br /><em>the room.</em></>} text="Sample quotes shown while client references are being collected — the care is real." /><div className="testimonial-grid">{testimonials.map((t, i) => <Reveal className="testimonial" key={t[2]} delay={i * 0.08} data-testid={`testimonial-${i + 1}`}><span className="quote-mark">“</span><p>{t[0]}</p><div className="testimonial-who"><span className="monogram">{t[1]}</span><div><strong>{t[2]}</strong><span>{t[3]}</span></div></div></Reveal>)}</div></section>; }

function Insights() { return <section className="section light insights-section"><SectionHead eyebrow="10 / INSIGHTS" title={<>Notes from<br /><em>the field.</em></>} text="The insights journal is on its way — a first look at what we are writing." /><div className="insight-grid">{insights.map((t, i) => <Reveal key={t[1]} delay={i * 0.08}>{t[3] ? <Link to={t[3]} className="insight-card insight-linked" data-testid={`insight-card-${i + 1}`}><span className="insight-tag">{t[0]}</span><h3>{t[1]}</h3><div className="insight-meta"><span>{t[2]}</span><span>READ NOW ↗</span></div></Link> : <div className="insight-card" data-testid={`insight-card-${i + 1}`}><span className="insight-tag">{t[0]}</span><h3>{t[1]}</h3><div className="insight-meta"><span>{t[2]}</span><span>COMING SOON</span></div></div>}</Reveal>)}</div></section>; }

function AboutTeaser() { return <section className="section light about-teaser"><Reveal><span className="eyebrow">11 / A POINT OF VIEW</span><h2>We don’t just<br />make things look good.<br /><em>We make them work.</em></h2></Reveal><Reveal delay={0.12}><p>The Squirrel Agency is a digital-first creative and performance studio for ambitious brands that want their next chapter to matter.</p><Link to="/about" className="button button-outline" data-testid="meet-agency-link">Meet the agency <span>↗</span></Link></Reveal></section>; }

function CTA() { return <section className="final-cta"><Reveal><span className="eyebrow">12 / YOUR NEXT CHAPTER</span><h2>Let’s build<br /><em>something</em><br />remarkable.</h2><p>Tell us what you’re building, what is not working, or what you want to make possible.</p><Link to="/contact" className="button button-accent" data-testid="final-start-project">Start a project <span>↗</span></Link></Reveal></section>; }
...
function ServicesPage() { return <><Header /><main><PageIntro eyebrow="02 / CAPABILITIES" title={<>The right people<br /><em>for the whole picture.</em></>} text="We bring the disciplines together early, so the output feels like one thought — not eleven handoffs." /><ServicesSection /><section className="service-manifesto"><Reveal><span className="eyebrow">TECHNOLOGY · CREATIVITY · PERFORMANCE</span><h2>One team.<br /><em>More momentum.</em></h2><Link to="/contact" className="button button-accent" data-testid="services-cta">Start a project <span>↗</span></Link></Reveal></section></main><Footer /></>; }

function ServiceDetail() { const { slug } = useParams(); const index = Math.max(0, services.findIndex(s => s[3] === slug)); const item = services[index] || services[0]; return <><Header /><main><PageIntro eyebrow={`${item[0]} / SERVICE DETAIL`} title={<>{item[1]}<br /><em>with intent.</em></>} text={item[2]} /><section className="light detail-hero"><Reveal><Image src={serviceVisuals[index]} alt={`${item[1]} detail visual`} /></Reveal><Reveal delay={0.1}><span className="eyebrow">THE OPPORTUNITY</span><h2>Make the next<br /><em>move clearer.</em></h2><p>We build the strategy, creative and delivery around the moment your audience is ready to act.</p><Link to="/contact" className="button button-accent" data-testid="service-detail-cta">Talk to us <span>↗</span></Link></Reveal></section><section className="light detail-columns"><Reveal><span className="eyebrow">WHAT YOU GET</span><h2>A sharper route<br /><em>to progress.</em></h2></Reveal><Reveal className="detail-points" delay={0.1}><p><b>01 / STRATEGY</b> A clear point of view that gives every decision a job.</p><p><b>02 / CRAFT</b> Work that is considered in the details and confident in the room.</p><p><b>03 / MOMENTUM</b> A measured way to learn, improve and keep moving.</p></Reveal></section></main><Footer /></>; }

function AboutPage() { return <><Header /><main><PageIntro eyebrow="03 / THE AGENCY" title={<>We make digital<br /><em>matter.</em></>} text="The Squirrel Agency is a digital-first creative, technology and performance marketing studio helping ambitious brands build a stronger presence and turn attention into growth." /><section className="light about-story"><Reveal className="about-photo"><Image src={images.office} alt="Modern creative agency interior" /></Reveal><Reveal delay={0.1}><span className="eyebrow">OUR PHILOSOPHY</span><h2>Ideas should<br /><em>do something.</em></h2><p>We believe the best digital work creates a feeling, removes friction and leaves a measurable trace. That means strategy without theatre, design with a point of view and technology that gets out of the way.</p><div className="beliefs"><span>01 / Curiosity over convention</span><span>02 / Clarity is a creative act</span><span>03 / Every detail earns its place</span></div></Reveal></section><section className="founder"><Reveal><span className="eyebrow">THE FOUNDER</span><h2>Akash<br /><em>Gupta.</em></h2><span className="founder-title">Founder & Creative Strategist</span></Reveal><Reveal className="founder-copy" delay={0.1}><Image src={images.founder} alt="Akash Gupta, Founder & Creative Strategist of The Squirrel Agency" /><p>Akash Gupta is the founder of The Squirrel Agency, a digital-first creative and performance marketing agency built around one simple belief — brands should not just be seen, they should be remembered.</p><p>With a strong interest in technology, creativity and digital growth, Akash brings together strategy, design, development and performance marketing to help businesses build a stronger digital presence and turn attention into measurable growth.</p></Reveal></section><CTA /></main><Footer /></>; }
